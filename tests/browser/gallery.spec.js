...
test('reduced-motion 默认静止，用户可主动开启', async ({ page }) => {
  ...
  await ready(page);
  const viewer = page.locator('#viewport');
  await expect(page.locator('#motion')).toHaveText('开启扇翅');
...
test('缩放上下限可控，反复切换后仍可重置', async ({ page }) => {
  ...
  await expect(viewer).toHaveAttribute('data-camera-distance', '5.400');
...
test('320px 窄屏无横向溢出，参考与艺术化说明可阅读', async ({ page }) => {
  await page.setViewportSize({ width: 320, height: 700 });
  await ready(page);
  expect(await page.evaluate(() => document.documentElement.scrollWidth <= innerWidth)).toBe(true);
  await page.locator('summary').click();
  await expect(page.locator('details a').first()).toBeVisible();
  await expect(page.locator('.disclaimer')).toContainText('并非科研复原');
